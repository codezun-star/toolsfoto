...
**Anuncios — Ezoic activo, Monetag desactivado (2026-07-09):**
- **Ezoic (`src/components/layout/EzoicScripts.astro`):** integración JavaScript Standalone. Contiene los scripts de privacidad (CMP Gatekeeper), `sa.min.js`, el init de `ezstandalone` y analytics — todos `is:inline`. Se incluye en el `<head>` de los 10 puntos con head propio: `ToolLayout.astro`, `LegalLayout.astro`, `index.astro`, `imagen/pdf/video/audio/developer.astro`, `blog/index.astro` y `blog/[slug].astro`. Los scripts de privacidad deben ir **siempre antes** de `sa.min.js`. El `/ads.txt` se sirve vía redirect 301 en `public/_redirects` → `srv.adstxtmanager.com/19390/toolsfoto.com`. Los placements se añaden con `ezstandalone.cmd.push(function(){ ezstandalone.showAds(...) })`.
- **Monetag (desactivado):** los 3 scripts de `src/components/layout/AdScripts.astro` (incluido vía `Footer.astro`) y la meta de verificación en los 5 heads antiguos están **comentados** con `{/* */}` por la aplicación al programa incubadora de Ezoic. No descomentar sin decisión explícita del usuario — Ezoic no permite redes de pop/redirect simultáneas.
