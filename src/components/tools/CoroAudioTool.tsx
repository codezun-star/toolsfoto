...
export default function CoroAudioTool() {
  ...
  async function process() {
    ...
    try {
      ...
      console.error('[CoroAudio] Error FFmpeg:', err);
      setError('Error al aplicar el efecto coro. Asegúrate de que el archivo es un formato de audio válido.');
    } finally {
      setProcessing(false);
    }
  }
  ...
  function download() {
    ...
    a.href = resultUrl;
    a.download = audio.name.replace(/\.[^.]+$/, `_coro.${ext}`);
    a.click();
  }
      ...
      {audio && !processing && !resultUrl && (
        <div className="space-y-4">
          <div className="p-5 bg-white rounded-xl border border-[var(--color-border)] space-y-3">
            <h2 className="font-bold text-[var(--color-text)]">Intensidad del coro</h2>
      ...
      {resultUrl && audio && (
        <div className="p-5 bg-white rounded-xl border border-[var(--color-tools-border)] space-y-4">
          <p className="text-sm text-[var(--color-text-secondary)]">
            ...
            Con coro: <strong className="text-[var(--color-accent)]">{formatBytes(resultSize)}</strong>
          </p>
