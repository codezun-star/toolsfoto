...
export default function AudioToMp3Base({ sourceLabel }: Props) {
  ...
  async function process() {
    ...
    try {
      ...
      const blob = await runFFmpeg(ff, audio.file, 'input_audio', ['-acodec', 'libmp3lame', '-b:a', `${bitrate}k`], 'output.mp3');
      setResultSize(blob.size);
      setResultUrl(URL.createObjectURL(blob));
      setProgress(100);
    } catch {
      setError(`Error al convertir el audio. Asegúrate de que el archivo es un ${sourceLabel} válido.`);
    } finally {
      setProcessing(false);
    }
  }
  ...
  function download() {
    ...
    a.href = resultUrl;
    a.download = audio.name.replace(/\.[^.]+$/, '.mp3');
    a.click();
  }

  return (
    <div className="space-y-6">
      <AudioUploader label={`Sube tu ${sourceLabel}`} onFile={setAudio} onClear={handleClear} current={audio} />
            ...
            <div className="grid grid-cols-4 gap-3">
              {BITRATES.map((b) => (
                <button
                  ...
                  className={['px-3 py-3 rounded-xl border text-sm font-semibold transition-colors', bitrate === b ? 'border-[var(--color-accent)] bg-[var(--color-accent-bg)] text-[var(--color-accent)]' : 'border-[var(--color-border)] hover:border-[var(--color-tools-border)] text-[var(--color-text-secondary)]'].join(' ')}
                >
                  {b} kbps
                </button>
              ))}
      ...
      {resultUrl && audio && (
        <div className="p-5 bg-white rounded-xl border border-[var(--color-tools-border)] space-y-4">
          <p className="text-sm text-[var(--color-text-secondary)]">MP3 generado ({bitrate} kbps): <strong className="text-[var(--color-accent)]">{formatBytes(resultSize)}</strong></p>
          <button onClick={download} className="w-full flex items-center justify-center gap-2 py-3 bg-[var(--color-accent)] text-white font-semibold rounded-xl hover:bg-[#C93D1E] transition-colors">
            <Download size={18} /> Descargar MP3
          </button>
