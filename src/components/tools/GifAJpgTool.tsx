...
export default function GifAJpgTool() {
  ...
  async function convert() {
    ...
    try {
      ...
      setError('Error al convertir el GIF. Prueba con otro archivo.');
    } finally {
      setLoading(false);
    }
  }
    ...
    <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
      <div className="space-y-4">
        ...
        {result && upload.image && (
          <div className="p-4 bg-white rounded-xl border border-[var(--color-border)] grid grid-cols-2 gap-3 text-xs text-[var(--color-text-muted)]">
            <div>
              <p className="mb-1.5">GIF original</p>
      ...
      <div className="space-y-5">
        <div className="p-5 bg-white rounded-xl border border-[var(--color-border)] space-y-4">
          <div className="flex items-start gap-2 p-2.5 bg-[var(--color-bg)] rounded-lg">
            <Info size={14} className="text-[var(--color-text-muted)] shrink-0 mt-0.5" />
            <p className="text-xs text-[var(--color-text-secondary)]">Si el GIF está animado, se convierte el primer fotograma a una imagen JPG estática.</p>
          </div>
          ...
          <div className="flex items-center gap-3">
            <label className="text-sm font-medium text-[var(--color-text)]">Fondo</label>
