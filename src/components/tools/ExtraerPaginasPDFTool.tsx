...
export default function ExtraerPaginasPDFTool() {
  ...
  const parsedPages = pdf && range.trim() ? parseRange(range, pageCount) : [];

  async function handleExtract() {
    if (!pdf || parsedPages.length === 0) return;
    setLoading(true);
    setError(null);
    try {
      const { PDFDocument } = await import('pdf-lib');
      ...
      const copied = await out.copyPages(src, parsedPages.map(p => p - 1));
    ...
    <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
      ...
      <div className="space-y-5">
        <div className="p-5 bg-white rounded-xl border border-[var(--color-border)] space-y-4">
          <h2 className="font-bold text-[var(--color-text)]">Páginas a extraer</h2>
          <input
            type="text"
            value={range}
            onChange={e => setRange(e.target.value)}
            placeholder={`Ej: 1-3, 5, 8-10 (de 1 a ${pageCount || 'N'})`}
            disabled={!pdf}
          ...
          {parsedPages.length > 0 && (
            <div className="p-3 bg-[var(--color-tools-bg)] rounded-lg border border-[var(--color-tools-border)]">
              <p className="text-xs text-[var(--color-text-secondary)]">
                Se extraerán <strong>{parsedPages.length} página{parsedPages.length !== 1 ? 's' : ''}</strong>: {parsedPages.join(', ')}
              </p>
            </div>
          )}
        </div>
        ...
        <DownloadButton
          ...
          disabled={!pdf || parsedPages.length === 0 || loading}
          loading={loading}
          label={parsedPages.length > 0 ? `Extraer ${parsedPages.length} página${parsedPages.length !== 1 ? 's' : ''}` : 'Extraer páginas'}
          className="w-full"
        />
      </div>
    </div>
