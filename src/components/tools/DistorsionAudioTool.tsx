...
const PRESETS = [
  {
    label: 'Overdrive suave',
    description: 'Distorsión ligera, cálida',
    filter: 'overdrive=gain=15:colour=15',
  },
  {
    label: 'Guitarra',
    description: 'Distorsión clásica de guitarra eléctrica',
    filter: 'overdrive=gain=35:colour=30',
  },
  {
    label: 'Bitcrush 8-bit',
    description: 'Efecto retro de videojuego 8 bits',
    filter: 'acrusher=level_in=4:level_out=1:bits=8:mode=log:aa=1',
  },
  {
    label: 'Heavy',
    description: 'Distorsión extrema y agresiva',
    filter: 'overdrive=gain=60:colour=50,highpass=f=200',
  },
];
...
export default function DistorsionAudioTool() {
  ...
  async function process() {
    ...
    try {
      ...
      console.error('[DistorsionAudio] Error FFmpeg:', err);
      setError('Error al aplicar la distorsión. Asegúrate de que el archivo es un formato de audio válido.');
    } finally {
      setProcessing(false);
    }
  }
  ...
  function download() {
    ...
    a.href = resultUrl;
    a.download = audio.name.replace(/\.[^.]+$/, `_distorsion.${ext}`);
    a.click();
  }
      ...
      {audio && !processing && !resultUrl && (
        <div className="space-y-4">
          <div className="p-5 bg-white rounded-xl border border-[var(--color-border)] space-y-3">
            <h2 className="font-bold text-[var(--color-text)]">Tipo de distorsión</h2>
      ...
      {resultUrl && audio && (
        <div className="p-5 bg-white rounded-xl border border-[var(--color-tools-border)] space-y-4">
          <p className="text-sm text-[var(--color-text-secondary)]">
            ...
            Con distorsión: <strong className="text-[var(--color-accent)]">{formatBytes(resultSize)}</strong>
          </p>
          ...
          <button onClick={download} className="w-full flex items-center justify-center gap-2 py-3 bg-[var(--color-accent)] text-white font-semibold rounded-xl hover:bg-[#C93D1E] transition-colors">
            <Download size={18} />
            Descargar audio con distorsión
          </button>
