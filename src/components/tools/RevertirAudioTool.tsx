...
export default function RevertirAudioTool() {
  ...
  async function process() {
    ...
    try {
      ...
      console.error('[RevertirAudio] Error FFmpeg:', err);
      setError('Error al revertir el audio. Asegúrate de que el archivo es un formato de audio válido.');
    } finally {
      setProcessing(false);
    }
  }
  ...
  function download() {
    ...
    a.href = resultUrl;
    a.download = audio.name.replace(/\.[^.]+$/, `_reverse.${ext}`);
    a.click();
  }
      ...
      {audio && !processing && !resultUrl && (
        <div className="space-y-4">
          ...
          <div className="p-3 bg-amber-50 border border-amber-200 rounded-xl text-xs text-amber-700">
            El audio se reproducirá de atrás hacia adelante. El resultado conserva el mismo formato que el archivo original.
          </div>
