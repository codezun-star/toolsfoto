...
const ROTATIONS = [
  { label: '90° derecha', filter: 'transpose=1' },
  { label: '90° izquierda', filter: 'transpose=2' },
  { label: '180°', filter: 'transpose=1,transpose=1' },
] as const;

export default function RotarVideoTool() {
  ...
  async function process() {
    ...
    try {
      ...
      setError('Error al rotar el vídeo. Comprueba que el formato es compatible.');
    } finally {
      setProcessing(false);
    }
  }
  ...
  function download() {
    ...
    a.href = resultUrl;
    a.download = video.file.name.replace(/\.[^.]+$/, '_rotado.mp4');
    a.click();
  }
    ...
    <div className="space-y-6">
      ...
      {video && !processing && !resultUrl && (
        <div className="space-y-5">
          <div>
            <p className="text-sm font-semibold text-[var(--color-text)] mb-3">Rotación</p>
            <div className="grid grid-cols-3 gap-2">
              {ROTATIONS.map((r, i) => (
                <button
                  key={i}
                  onClick={() => setRotation(i)}
                  className={['px-3 py-3 rounded-xl border text-sm font-medium transition-colors', rotation === i ? 'border-[var(--color-accent)] bg-[var(--color-accent-bg)] text-[var(--color-accent)]' : 'border-[var(--color-border)] text-[var(--color-text-secondary)]'].join(' ')}
                >
                  {r.label}
                </button>
              ))}
      ...
      {processing && (
        <div className="p-6 text-center">
          ...
          <p className="text-sm mt-3">{progress === 0 ? 'Cargando procesador…' : `Rotando vídeo… ${progress}%`}</p>
        </div>
      )}
